"""Android platform - Flet UI with Flet 0.86.5"""
import flet as ft
import threading
...
from common.services.llm import test_connection
...
class AndroidPlatform(Platform):
    """Android platform - Flet implementation"""
    ...
    def run(self):
        ...
        def main(page: ft.Page):
            self.app = DictationApp(self.storage, page)
            self.app.init_ui(page)
        
        ft.app(target=main)
    
    def run_web(self, port=8500):
        """Run Flet app in web mode for testing"""
        def main(page: ft.Page):
            self.app = DictationApp(self.storage, page)
            self.app.init_ui(page)
...
class DictationApp:
    ...
    def __init__(self, storage, page=None):
    ...
    def init_ui(self, page: ft.Page):
        ...
        # Streaming UI components
        self.thinking_text = ft.Text("", size=12, color=TEXT_SECONDARY, selectable=True)
        self.stream_status = ft.Text("", size=11, color=TEXT_SECONDARY)
        self.stream_progress = ft.ProgressBar(width=300, color=ACCENT_GREEN, value=0)
        self.streaming_container = ft.Container(
            content=ft.Column([
                self.stream_status,
                self.stream_progress,
                ft.Text("Thinking...", size=11, color=TEXT_SECONDARY, visible=False),
                self.thinking_text,
            ], spacing=4), padding=10, bgcolor=BG_CARD, border_radius=8, visible=False
        )
    ...
    def _build_passage(self):
        """Build passage tab with streaming support"""
    ...
    def _build_settings(self):
        """Build settings tab - load existing config"""
    ...
    def _start_session(self):
        """Start dictation session - focus on wrong + pending words"""
    ...
    def _start_full_library_session(self, e):
        """Start full library review session"""
    ...
    def _gen_passage(self, e):
        """Generate passage using shared streaming service"""
        self.passage_loading.visible = False
        self.passage_card.visible = False
        self.streaming_container.visible = True
        self.stream_status.value = "准备中..."
        ...
        wrong_words_data = self.storage.get_wrong_words()
        wrong_words = [w['word'] for w in wrong_words_data if w.get('error_count', 0) >= 2][:5]
        
        provider_config = self.storage.get_active_provider()
            ...
            try:
                for event in generate_passage_stream(
                    ...
                    if event_type == 'status':
                        self.stream_status.value = event['message']
                        self.page.update()
                    elif event_type == 'debug_prompt':
                        self.stream_status.value = f"Model: {event.get('model', 'unknown')}"
                        self.page.update()
                    elif event_type == 'thinking_start':
                        self.stream_status.value = "Thinking..."
                        self.page.update()
                    elif event_type == 'thinking':
                        self.thinking_text.value = event['content'][-500:]
                        self.stream_progress.value = 0.3
                        self.page.update()
                    elif event_type == 'text_start':
                        self.stream_status.value = "Generating..."
                        self.page.update()
                    elif event_type == 'text':
                        self.p_content.value = event['content']
                        self.stream_progress.value = 0.6
                        self.page.update()
                    elif event_type == 'done':
                        result = event['result']
                        self.p_title.value = "Generated Passage"
                        self.p_content.value = result.get('content', '')
                        self.p_trans.value = result.get('translation', '')
                        self.stream_progress.value = 1.0
                        self.stream_status.value = f"Done! {len(result.get('words_used', []))} words used"
                        self.passage_card.visible = True
                        self.page.update()
                    elif event_type == 'error':
                        self.p_title.value = "Generation Failed"
                        self.p_content.value = f"Error: {event['message']}"
                        self.p_trans.value = ""
                        self.passage_card.visible = True
                        self.page.update()
                        return
    ...
    def _save_settings(self, e):
        ...
        if not provider:
            self.storage.add_provider(
                name='Default', provider_type='anthropic',
                auth_token=self.auth_token.value, model=self.model_f.value,
                max_tokens=int(self.max_t.value or 1024),
                temperature=float(self.temp_f.value or 0.7),
                base_url=self.base_url.value, is_active=True
            )
        else:
            self.storage.update_provider(
                provider['id'], auth_token=self.auth_token.value,
                model=self.model_f.value, max_tokens=int(self.max_t.value or 1024),
                temperature=float(self.temp_f.value or 0.7),
